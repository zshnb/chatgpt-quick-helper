...
import {MessageItem, Voice} from "@/types.ts";
import {getMessageIds} from "@/entrypoints/utils/message-util.ts";
import Spin from "@/entrypoints/components/spin/Spin.tsx";
import LoadingButton from "@/entrypoints/components/button/LoadingButton.tsx";
import Mask from "@/entrypoints/components/mask/Mask.tsx";
...
const App = () => {
  ...
  useEffect(() => {
    browser.storage.local.get(['voice']).then(async (res) => {
      if (!res.voice) {
        await browser.storage.local.set({ voice: voice.value });
      }
    });
  }, []);
  ...
  useEffect(() => {
    async function getMessages() {
      const storage = await browser.storage.local.get(['conversationId', 'jwt']);
      const conversationId = storage.conversationId;
      const jwt = storage.jwt;
      console.log(`conversationId: ${conversationId}, jwt: ${jwt}`)
        ...
        <form className={'flex flex-col gap-y-2 relative'}>
          ...
          <Select
            ...
            onChange={async (e: any) => {
              setVoice(e.value);
              await browser.storage.local.set({ voice: e.value });
            }}
          ></Select>
          <button
